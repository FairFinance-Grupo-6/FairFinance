export function calcularDiasDescuento(
  ...
  const startDate = new Date(fechaEmision).getTime();
  const endDate = new Date(fechaVencimiento).getTime();
  return Math.ceil((endDate - startDate) / (1000 * 3600 * 24));
}

export function calcularSumaCostos(
  costosAdicionales: { descripcion: string; monto: number }[],
): number {
  return costosAdicionales.reduce((acc, costo) => acc + costo.monto, 0);
}

function calcularSumaCostosAdicionales(
  costosAdicionales: {
    descripcion: string;
    monto: number;
  ...
  return costosAdicionales
    .filter((costo) => costo.pagadoAlInicio === pagadoAlInicio)
    .reduce((acc, costo) => acc + costo.monto, 0);
}
...
function calcularTasaDescuento(
  ...
  if (tipoTasa === "Efectiva") {
    TEPlazoDeDescuento =
      Math.pow(1 + valorTasa / 100, diasDescuento / diasPeriodo) - 1;
  } else if (tipoTasa === "Nominal") {
    TEPlazoDeDescuento =
      Math.pow(
        1 + valorTasa / 100 / diasPeriodo,
        diasDescuento / obtenerDiasPeriodo(capitalizacion!),
      ) - 1;
  } else {
...
export function calcularDescuento(
  ...
  const TEPlazoDeDescuento = calcularTasaDescuento(
    diasDescuento,
    valorTasa,
    diasPeriodo,
    tipoTasa,
...
export function calcularTcea(
  ...
  const tcea =
    100 * (Math.pow(valorEntregado / valorRecibido, 360 / diasDescuento) - 1);
  console.log("tca", tcea);
  return tcea;
}

export function calcularTceaConMora(
  importeNominal: number,
  descuento: number,
  retencion: number,
  costosAdicionales: {
  ...
  const costosInicio = calcularSumaCostosAdicionales(costosAdicionales, true);
  const costosFin = calcularSumaCostosAdicionales(costosAdicionales, false);
  const costosAdicionalesConMora = costosMora.reduce(
    (sum, c) => sum + c.monto,
    0,
  );
  const valorRecibido = valorNeto - costosInicio - retencion;
  ...
  const valorEntregadoMora =
    valorEntregado + costosAdicionalesConMora + interesesMora + interesesComp;
  const tceaMora =
    (Math.pow(
      valorEntregadoMora / valorRecibido,
      360 / (diasDescuento + diasMora),
    ) -
      1) *
    100;
