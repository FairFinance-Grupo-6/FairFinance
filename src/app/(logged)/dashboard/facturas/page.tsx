...
export default function FacturasPage() {
    ...
    autoTable(doc, {
      ...
      body: filteredFacturas.map((invoice) => [
        ...
        invoice.costosAdicionales
          .map((cost: { descripcion: any; monto: any }) => `${cost.descripcion}: ${safeToFixed(cost.monto)}`)
          .join(", "),
        invoice.tipoTasa,
        invoice.tiempoTasa,
  ...
  doc.text(`TCEA Promedio: ${promedioTCEA.toFixed(2)}%`, 20, (doc as any).lastAutoTable.finalY + 10);
    ...
    <main className="w-full max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="flex justify-between items-center mb-4 relative">
        ...
        <Link href="/dashboard/facturas/nueva">
          <button className="bg-[#5756BB] hover:bg-[#8182DA] hover:scale-105 transition-all duration-300 text-white px-4 py-2 rounded">
            Nueva Factura
          </button>
        </Link>
      </div>
      ...
      <div className="flex items-center justify-between mt-4 space-x-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            ...
            disabled={currentPage === Math.ceil(filteredFacturas.length / invoicesPerPage)}
          >
            &gt;
          </button>
        </div>
